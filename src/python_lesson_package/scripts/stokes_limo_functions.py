...
def Drive(velocity, timeSet):
	timePassed = 0
	while timePassed < timeSet:
		limo.SetMotionCommand(velocity / 100, 0, 0)
		time.sleep(0.1)
		timePassed += .1


def Turn(velocity, timeSet):
	timePassed = 0
	while timePassed < timeSet:
		limo.SetMotionCommand(0, velocity / 100, 0)
		time.sleep(0.1)
		timePassed += .1

def Strafe(velocity, timeSet):
	timePassed = 0
	while timePassed < timeSet:
		limo.SetMotionCommand(0, 0, velocity / 100)
		time.sleep(0.1)
		timePassed += .1
